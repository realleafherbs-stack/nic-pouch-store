  ...
  return records.flatMap((record) => {
    ...
    return [{
      ...
      flavor: typeof attributes.flavor === "string" && attributes.flavor.trim()
        ...
        : inferFlavorFromName(name, brand),
      nicotineMg,
      strengthLevel,
      retailPrice,
      ...(mapPriceTiers(item.priceTiers) ? { priceTiers: mapPriceTiers(item.priceTiers) } : {}),
      sourcePrice: finiteNumber(attributes.sourcePrice) ?? 0,
      // The CRM doesn't track real stock quantity yet, so treat everything it
      // returns as available rather than showing every synced product as out
      // of stock.
