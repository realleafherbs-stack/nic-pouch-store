...
export function crmCatalogSyncEnabled(environment = process.env) {
  if (environment.CRM_CATALOG_SYNC === "false") return false;
  return Boolean(
    environment.CRM_API_BASE_URL?.trim()
    && environment.CRM_SITE_SLUG?.trim()
    && environment.CRM_API_KEY?.trim()
  );
}
...
const HEBREW_SLUG_PHRASES = [
  ["פסיפלורה מתוקה", "sweet-passionfruit"],
  ["לימונדה כחולה", "blue-lemonade"],
  ["בריזה טרופית", "tropical-breeze"],
  ["בלאק דאבל פרש", "black-double-fresh"],
  ["בלאק ווטרמלון", "black-watermelon"],
  ["בלאק קולד דריי", "black-cold-dry"],
  ["וויט דאבל פרש", "white-double-fresh"],
  ["וויט ווטרמלון", "white-watermelon"],
  ["וויט קולד דריי", "white-cold-dry"],
  ["בלוברי אקסטרים", "blueberry-extreme"],
  ["דובדבן אקסטרים", "cherry-extreme"],
  ["מנטה אקסטרים", "mint-extreme"],
  ["ענבים אקסטרים", "grape-extreme"],
  ["טרופיקל פאנצ'", "tropical-punch"],
  ["טרופיקל פאנץ'", "tropical-punch"],
  ["דובדבן קרח", "cherry-ice"],
  ["ענבים קרח", "grape-ice"],
  ["ענבים אייס", "grape-ice"],
  ["מנגו אייס", "mango-ice"],
  ["פטל כחול", "blue-raspberry"],
  ["פטל שחור", "blackberry"],
  ["באבל גאם", "bubble-gum"],
  ["לימון מנטה", "lemon-mint"],
  ["בלאק בלוברי", "black-blueberry"],
  ["בלאק גרייפ", "black-grape"],
  ["בלאק צ'רי", "black-cherry"],
  ["וויט בלוברי", "white-blueberry"],
  ["וויט גרייפ", "white-grape"],
  ["וויט צ'רי", "white-cherry"],
  ["קולד מינט", "cold-mint"],
  ["בד אפל", "bad-apple"],
  ["בלו אייס", "blue-ice"],
  ["קול חזק", "strong-cool"],
...
function transliterateProductSlug(value) {
  ...
  for (const [hebrew, english] of HEBREW_SLUG_PHRASES) {
    normalized = normalized.replaceAll(hebrew, english);
  }
...
// Static-export routes need ASCII-safe URLs. The public URL should contain
// only the semantic product identity. Collision handling happens after the
// complete CRM set is mapped, where we can prove whether a suffix is needed.
function safeSlug(name, id, brand) {
  const ascii = transliterateProductSlug(name);
...
function explicitProductSlug(item) {
  ...
  if (canonicalSlug && /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(canonicalSlug)) return canonicalSlug;
...
export function mapCrmProducts(records) {
  ...
  const products = records.flatMap((record) => {
    ...
    const parsedPrice = finiteNumber(item.price);
    ...
    const generatedSlug = safeSlug(name, id, brand);
    const slug = explicitProductSlug(item) ?? generatedSlug;

    if (!id || !slug || !name || parsedPrice === null || parsedPrice < 0 || packSize !== 1) return [];
    const sellable = parsedPrice > 0 && item.active !== false;
    ...
    const legacySlugs = uniqueStrings([
      ...(Array.isArray(item.previousHandles) ? item.previousHandles : []),
      ...(generatedSlug !== slug ? [generatedSlug] : []),
      ...(canonicalSlug && canonicalSlug !== slug ? [canonicalSlug] : []),
    ]).filter((legacySlug) => legacySlug !== slug);
    ...
    return [{
      ...
      stock: sellable ? (stock === null ? 999 : Math.max(0, Math.trunc(stock))) : 0,
      active: sellable,
      packSize: 1,
      images,
  ...
  // Payper can retain an older SKU after a replacement SKU is created. Two
  // indexable pages with the same normalized name compete for the same query
  // and show the customer duplicate products. Keep every CRM record in the
  // build snapshot, but expose only the strongest record for each exact
  // product identity (prefer a real image, tracked stock and source price).
  const byIdentity = new Map();
  for (const product of products) {
    if (!product.active) continue;
    const identity = `${product.brand}|${product.name}|${product.nicotineMg ?? ""}`.toLowerCase();
    const score = (product.images.length ? 1_000_000 : 0)
      + (product.stock !== 999 ? 100_000 : 0)
      + (product.sourcePrice > 0 ? 10_000 : 0)
      + Math.min(product.stock, 9_999);
    const current = byIdentity.get(identity);
    if (!current || score > current.score) byIdentity.set(identity, { product, score });
  }

  const deduplicated = products.map((product) => {
    if (!product.active) return product;
    const identity = `${product.brand}|${product.name}|${product.nicotineMg ?? ""}`.toLowerCase();
    const winner = byIdentity.get(identity)?.product;
    if (!winner || winner.id === product.id) return product;
    return {
      ...product,
      slug: `${product.slug}-${product.id.slice(-8)}`,
      active: false,
      stock: 0,
      indexable: false,
      canonicalUrl: `/shop/${winner.slug}`,
    };
  });

  const activeSlugCounts = new Map();
  for (const product of deduplicated) {
    if (product.active) activeSlugCounts.set(product.slug, (activeSlugCounts.get(product.slug) ?? 0) + 1);
  }

  return deduplicated.map((product) => {
    if (!product.active || activeSlugCounts.get(product.slug) === 1) return product;
    const slug = `${product.slug}-${product.id.slice(-8)}`;
    return { ...product, slug, canonicalUrl: `/shop/${slug}` };
  });
}
