...
const asciiSlug = (value) => value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
const products = data
  ...
  .map((row) => {
    ...
    return {
      id: row["מזהה"], slug: [asciiSlug(brand), nicotineMg ? `${String(nicotineMg).replace(".", "")}mg` : null, row["מזהה"]].filter(Boolean).join("-"),
